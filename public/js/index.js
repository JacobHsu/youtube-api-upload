$(function() {
    ...
    socket.on('done', function(video){
      var liContent = '<li class="collection-item avatar"><i class="material-icons circle red">play_arrow</i><span class="title">Title</span><p>'+ video.snippet.title+'</p></li>'

      var playerEmbedHtml = '<iframe width="560" height="315" src="https://www.youtube.com/embed/'+ video.id+'" frameborder="0" allowfullscreen></iframe>'

      if($('#recentUploads').has('p')){ 
        $('#empty').remove(); // Remove "No videos Uploaded Yet"
      }
      $('#uploads').append(liContent).append(playerEmbedHtml);
